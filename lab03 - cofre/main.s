...
;<var>	SPACE <tam>                        ; Declara uma variável de nome <var>
                                           ...
                                           ; posição da RAM	
ARRAY_PW 	SPACE 2 ; (2 bytes) senha do usuario
INPUT_PW	SPACE 2	; (2 bytes) senha auxiliar
NUM_ATTEMPTS SPACE 1 ; (1 byte) numero de tentativas
IS_LOCKED	SPACE 1	; (1 byte) define se cofre esta travado ou nao
...
;                  código
		...
		EXPORT IS_LOCKED[DATA,SIZE=1] ; exporta para tratamento da interrupcao
...
Start
    ...
    BL SysTick_Init	;inicializa timer para delay
    BL LCD_init		;inicializa LCD
    BL MKBOARD_init	;inicializa matrix keyboard
    BL LEDS_AND_DISPLAYS_init	;inicializa leds e displays da placa TIVA
    BL SW1_SW2_init				;inicializa SW1 e SW2 nativo da placa
	
	; leitura e escrita em variavel de 1 byte
	LDR R0, =IS_LOCKED
	MOV R1, #0
	STRB R1, [R0]
	
	; leitura e escrita em variavel de 2 bytes
	LDR R0, =ARRAY_PW
	MOV R1, #0xABCD
	STRH R1, [R0]
...
; ========================
; Definicoes das strings utilizadas
