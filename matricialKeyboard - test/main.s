...
MainLoop
    BL LerTeclado     ; Read the keyboard
    MOV R1, R0         ; Store the pressed key in R1
    CMP R1, #0x10      ; Check if no key is pressed (0x10 indicates no key)
    BEQ MainLoop       ; If no key is pressed, loop again

    BL KeyCode_to_ASCII ; Convert the key code to ASCII
    BL LCD_DATA         ; Display the ASCII character on the LCD
